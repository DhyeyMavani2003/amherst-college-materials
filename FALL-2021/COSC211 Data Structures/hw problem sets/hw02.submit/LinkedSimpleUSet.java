public class LinkedSimpleUSet<E> implements SimpleUSet<E> {
    ...
    // size method
    public int size() {
        return this.size; // returning size
    }
    ...
    public E find(E x) {
        ...
        for (int i = 0; i < size; i++) { // search in the list
            E y = cur.value; //storing value in variable y
            if (x.equals(y)) { // if we found the semantically equal element
                return y;
            }
            cur = cur.next; // shift our cur node to next to progress in the list
        }
    ...
    public boolean add(E x) {
        ...
        if (size == 0) {
            ...
            for (int i = 0; i < size; i++) { // traversing along the list to check if we don't have a semantically equal elemnt already in the list
                E y = cur.value; // storing value in variable y
                if (x.equals(y)) {// if we found the semantically equal element
                    return false;
                }
